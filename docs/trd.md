...
2. The user types: full name, **full 12-digit IC** (Malaysian MyKad), monthly income RM, employment type (gig vs. salaried), optional address, and a dynamic list of dependants (relationship + age + optional IC last-6). Household size is derived on the server as `1 + len(dependants)` and never entered directly.
...
6. The 12-digit IC enters request-scope memory only. `build_profile_from_manual_entry::_parse_ic` splits it: the `YYMMDD` prefix (with two-digit-year disambiguation against the current date in MYT) becomes a real `date`, then `age`; the trailing `PB-####` becomes `Profile.ic_last6`. The full IC and the derived DOB are NOT written onto the `Profile` — only `age` and `ic_last6` are. Future scheme integrations (MyKasih, BUDI95) requiring the full IC re-prompt at the boundary rather than persisting it.
