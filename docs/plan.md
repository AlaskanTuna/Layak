...
- [ ] Deploy the backend with `--set-secrets=FIREBASE_ADMIN_KEY=firebase-admin-key:latest` and keep `firestore.rules` client-only access locked to the owner. _(Deferred: deploy command recorded in `docs/runbook.md` §2. Preconditions (service account created, key minted, secret populated) are user-driven gcloud commands; live deploy waits until the Phase 2 Task 3 frontend can attach a real ID token so the whole integration smokes green end-to-end.)_
