...
- **nearby suggestions.** The deterministic path probes nearby income/dependant changes and returns at most a few `WhatIfSuggestion` chips such as a lower-income or +1 dependant scenario that would materially change a scheme output. These are deterministic probes, not model inference.
...
- **benchmark evidence.** Local Vertex-backed benchmark data from 13 May 2026 is stored under `tmp/benchmarks/what-if/2026-05-13/` (gitignored). The clean runner `scripts.benchmark_what_if:run_what_if_gemini_classify_only` measured deterministic preview medians of 0.71ms-1.31ms versus Gemini classification-only medians of 1194ms-1719ms across the recorded runs, with measured speedups still above 1000x in the rerun. A full legacy blocking-path probe including strategy took 25s-47s per scenario; Discovery Engine was disabled in that benchmark project, so those full-legacy timings are evidence against blocking advisory generation, not production parity numbers.
...
- **BUDI95 rule.** `app/rules/budi95.py`. Eligibility: `age >= 16`. Citations: MOF 30 Sep 2025 press release (eligibility + RM1.99 price), Maybank2u explainer (300 L monthly cap), MOF Feb 2026 14.8M-users statement (program reach). Portal: `https://www.budi95.gov.my/`.
