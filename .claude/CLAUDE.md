...
- **Models:** `FAST_MODEL = gemini-3.1-flash-lite` (extract); `WORKER_MODEL = gemini-3.1-flash-lite` (classify); `HEAVY_MODEL = gemini-2.5-pro` (compute_upside); `HEAVY_MODEL_FALLBACK = gemini-2.5-pro`; `ORCHESTRATOR_MODEL = gemini-2.5-pro` reserved for an optional ADK runner cutover.
